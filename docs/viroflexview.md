...
###
backgroundColorType| Description  
---|---  
| **ColorPropType** The background color for the flexbox. The default
background color is transparent.Valid color formats are: _ '#f0f' (#rgb) _
'#f0fc' (#rgba) _ '#ff00ff' (#rrggbb) _ '#ff00ff00' (#rrggbbaa) _ 'rgb(255,
255, 255)' _ 'rgba(255, 255, 255, 1.0)' _ 'hsl(360, 100%, 100%)' _ 'hsla(360,
100%, 100%, 1.0)' _ 'transparent' _ 'red' * 0xff00ff00 (0xrrggbbaa)  
###
...
---|---  
| **PropTypes.number** The width of the ViroFlexView in 3D space. Default is
1.  
###
...
---|---  
| A bit mask that is bitwise and-ed (&) with every light's influenceBitMask
and if the result is > 0, then the light will apply to the object. For more
information please see the [Lighting and Materials](lighting-and-
materials.html) Guide.  
###
materialsType| Description  
---|---  
|
**PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.string),PropTypes.string])**
Since ViroFlexView is a 2D container, it can take an array of 1 material that
can serve as a background to the entire ViroFlexView.  
###
...
view. }}`The position parameter represents the position in world coordinates
on the object where the click occurred. For the mapping of sources to
controller inputs, see the [Events](input-events.html) section.  
###
...
container. A value of 1 translates into a fully opaque view while 0 represents
full transparency.  
###
positionType| Description  
---|---  
| **PropTypes.arrayOf(PropTypes.number)** Cartesian position of the container
in 3D world space, specified as [x, y, z].  
###
physicsBodyType| Description  
---|---  
|
**PropTypes.shape({..[physics.api](http://docs.viromedia.com/v2.0.0/docs/physics#physicsbody-
...
[physics.api](http://docs.viromedia.com/v2.0.0/docs/physics#physicsbody-api).  
###
...
---|---  
| **PropTypes.arrayOf(PropTypes.number)** The scale of the  in 3D space,
specified as [x,y,z]. A scale of 1 represents the current size of the . A
scale value of 1 will
make the  proportionally bigger along the specified axis.  
###
...
---|---  
| A bit mask that is bitwise and-ed (&) with lights' influenceBitMasks to
determine which lights on this object should apply shadows. For more
information see the [Lighting and Materials](lighting-and-materials.html)
Guide.  
###
styleType| Description  
---|---  
| **stylePropType** Please see [Styles](styles.html) for more details  
###
transformBehaviorsType| Description  
---|---  
|
**PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.string),PropTypes.string])**
An array of transform constraints that affect the transform of the
ViroFlexView. For example, putting the value "billboard" will ensure the
ViroFlexView is facing the user as the user rotates their head on any axis.
This is useful for having the ViroFlexView always face the user on a
particular axis, which especially useful for menus other 2D UI.Allowed
values(values are case
...
---|---  
| **PropTypes.bool** False if the ViroFlexView and it's children should be
hidden. By default the ViroFlexView is visible and this value is true.  
###
