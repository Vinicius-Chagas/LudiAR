...
MPEG videos accepted.To invoke with remote url:
`{uri:"http://example.org/myvideo.mp4"}`To invoke with local
...
---|---  
| **PropTypes.number** The height of the video in 3D space. Default value is
1.  
###
...
Takes array of 1 material that serves as the material of the surface that is
...
---|---  
| **PropTypes.func** Called when an object has been clicked.Example code:`
_onClick(position, source) { // user has clicked the object}`For the mapping
...
---|---  
| **PropTypes.func** Called when the user hovers on or off the control.For
example:` _onHover(isHovering, position, source) { if(isHovering) { // user is
hovering over the box } else { // user is no longer hovering over the box
...
---|---  
| **PropTypes.func** Called when the user performs a pinch gesture on the
control. When the pinch starts, the scale factor is set to 1 is relative to
the points of the two touch points. For example:` _onPinch(pinchState,
...
button. A value of 1 translates into a fully opaque button while 0 represents
full transparency.  
###
...
---|---  
| **PropTypes.arrayOf(PropTypes.number)** Cartesian position of the video in
3D world space, specified as [x, y, z].  
###
...
---|---  
| **PropTypes.bool** False if the video should be hidden. By default the video
...
---|---  
| **PropTypes.number** The width of the video in 3D space. Default value is 1.  
###
